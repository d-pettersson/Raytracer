...
TEST_F(IntersectionFixture, EncapsulationTest) {
    * intersection = raytracer::Intersection(3.5, sphere);
    EXPECT_EQ(3.5, intersection->getDistance());
...
TEST_F(IntersectionFixture, AggregationTest) {
    ...
    * xs = intersections(* intersection, * intersection2);
    ...
    ASSERT_EQ(xs[0][1].getDistance(), 2);
}

TEST_F(IntersectionFixture, Hit1) {
    * intersection = raytracer::Intersection(1, sphere);
    * intersection2 = raytracer::Intersection(2, sphere);
    * xs = intersections(* intersection2, * intersection);
    * hitIntersection = hit(* xs);
    EXPECT_EQ(hitIntersection->getDistance(), intersection->getDistance());
...
TEST_F(IntersectionFixture, Hit2) {
    ...
    * xs = intersections(* intersection2, * intersection);
    * hitIntersection = hit(* xs);
    EXPECT_EQ(hitIntersection->getDistance(), intersection2->getDistance());
...
TEST_F(IntersectionFixture, Hit3) {
    ...
    * xs = intersections(* intersection2, * intersection);
    * hitIntersection = hit(* xs);
    EXPECT_EQ(hitIntersection->getDistance(), 0);
...
TEST_F(IntersectionFixture, Hit4) {
    ...
    * xs = intersections(* intersection, * intersection2, * intersection3, * intersection4);
    * hitIntersection = hit(* xs);
    EXPECT_EQ(hitIntersection->getDistance(), intersection4->getDistance());
...
TEST_F(IntersectionFixture, FindingN1N2VariousIntersections) {
    ...
    * ray = raytracer::Ray(raytracer::createPoint(0, 0, -4), raytracer::createVector(0, 0, 1));
    * xs = intersections(A1, B1, C1, B2, C2, A2);
...
TEST_F(IntersectionFixture, SchlickApproximationUnderTotalInternalReflection) {
    ...
    * intersectionData = intersection2->prepareComputations(* ray, * xs);
    ASSERT_EQ(1.0, intersectionData->schlick());
}
...
TEST_F(IntersectionFixture, SchlickApproximationWithPerpendicularViewAngle) {
    ...
    * xs = intersections(* intersection, * intersection2);
    * intersectionData = intersection2->prepareComputations(* ray, * xs);
    ASSERT_FLOAT_EQ(0.04, intersectionData->schlick());
}
...
TEST_F(IntersectionFixture, SchlickApproximationWithSmallAngle) {
    sphere->createGlass();
    * ray = raytracer::Ray(raytracer::createPoint(0, 0.99, -2), raytracer::createVector(0, 0, 1));
    * intersection = raytracer::Intersection(1.8589, sphere);
    * xs = intersections(* intersection);
    * intersectionData = intersection->prepareComputations(* ray, * xs);
    ASSERT_NEAR(0.48873, intersectionData->schlick(), 0.001);
}
